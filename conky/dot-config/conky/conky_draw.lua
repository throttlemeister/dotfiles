...
function get_critical_or_not_suffix(
	value,
	threshold,
...
function draw_line(display, element)
	...
	if not element.graduated then
		...
		if element.number_graduation == 0 or element.space_between_graduation == 0 then
			error(
				"The values of number_graduation and space_between_graduation must be non-zero as they are used as divisors. Dividing by zero causes undefined behavior (usually it results in an 'inf')."
			)
		end
...
function draw_bar_graph(display, element)
	...
	if element.max_value == 0 then
		error(
			"The value of max_value must be non-zero as it is used a divisor. Dividing by zero causes undefined behavior (usually it results in an 'inf')."
		)
	end
...
function draw_ring(display, element)
	...
	if not element.graduated then
		...
		if element.number_graduation == 0 then
			error(
				"The value of number_graduation must be non-zero as it is used as a divisor. Dividing by zero causes undefined behavior (usually it results in an 'inf')."
			)
		end
